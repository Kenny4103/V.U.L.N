...
def clamd_scan(file_path):
    try:
        ...
        result = subprocess.run(['clamdscan', file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=True)
        print(result.stdout)
        ...
        return f"An error occurred while scanning {file_path}: {e.stdout}"
